import { assertSnapshotOwnership } from './completion_binding';
...
export async function replaceCanonicalAnalysis(ctx: any, args: { runId: any; repositoryKey: string; sourceCommitSha:string; analysis: any }) {
  ...
  if (args.sourceCommitSha.toLowerCase() !== analysis.snapshot.sourceCommitSha.toLowerCase()) throw new Error('Analysis snapshot commit does not match the bound attempt commit');

  for (const table of ['graphNodes','graphEdges','flows','principles','findings','dependencies','editorDiagnostics']) {
  ...
  for (const principle of analysis.principles ?? []) await ctx.db.insert('principles', { runId:args.runId,principleId:principle.id,statement:principle.statement,scope:{nodeIds:principle.scopeNodeIds,flowIds:principle.scopeFlowIds},provenance:principle.provenance,evidence:principle.evidence,confidence:principle.confidence,status:principle.status,approvedExceptions:principle.approvedExceptionIds,lastValidatedAt:principle.lastValidatedAt });
  ...
  if (existingSnapshot) {
    assertSnapshotOwnership(existingSnapshot, args.repositoryKey, analysis.snapshot.sourceCommitSha);
