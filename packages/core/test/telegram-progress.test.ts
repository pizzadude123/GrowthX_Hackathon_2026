...
describe('Telegram progress', () => {
  it.each(['mapping', 'auditing', 'validating', 'retrying', 'completed', 'blocked', 'cancelled'] as const)('formats a clear %s update', (stage) => {
    const message = formatTelegramProgress({ ...base, stage, mappedFiles: 12, flowCount: 3, findings: 2, elapsedMs: 45_000, detail: stage === 'blocked' ? 'Hermes unavailable' : undefined });
    expect(message).toContain('WB-TEST');
    expect(message.toLowerCase()).not.toMatch(/repair|publish|fixed|change was applied/);
  ...
  it('labels an incomplete structured handoff as partial rather than complete', () => {
    const message = formatTelegramFinalResult({
      ...
      resultState: 'partial', dashboardUrl: 'https://whitebox.example/runs/WB-PARTIAL',
    });
  ...
  it('includes evidence and agents without fabricating run-scoped eval results', () => {
    const message = formatTelegramFinalResult({
      ...
      agents: [{ role: 'Logic Auditor', status: 'completed' }],
    ...
    expect(message).not.toContain('Run gates:');
    expect(message).toContain(base.dashboardUrl);
