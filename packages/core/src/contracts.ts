...
export const runStatusSchema = z.enum(['accepted','mapping','auditing','validating','completed','partial','audit_only','blocked','failed','cancelled']);
export const runModeSchema = z.literal('audit_only');
...
export const findingSchema = z.object({
  ...
  violatedInvariantIds: z.array(z.string()), businessImpact: z.string(), recommendation: z.string(), repairability: z.literal('human_required'), rejectionReason: z.string().optional()
...
export const dependencyHealthSchema = z.object({
  ...
  deprecated: z.boolean(), classification: z.enum(['unverified','healthy','aging','majorly_behind','deprecated','abandoned_signal']), importingFiles: z.array(z.string()), affectedNodeIds: z.array(z.string()), affectedFlowIds: z.array(z.string()),
  probableUpgradeComplexity: z.enum(['unknown','low','medium','high']), safeAction: z.enum(['none','patch_update','minor_update','manual_migration','replace_package']), evidenceSummary: z.string()
});
...
export type EditorDiagnostic = { id:string; runId:string; findingId:string; path:string; startLine:number; endLine:number; severity:'information'|'warning'|'error'; title:string; flowLabel:string; principleStatement:string; impact:string; dashboardUrl:string; githubPermalink?:string };
...
export type RepositoryManifest = { files:ManifestFile[]; languages:{language:string; files:number; percentage:number}[]; ecosystems:string[]; manifests:string[]; validationCommands:string[]; unsupportedRegions:string[]; capability:{baseline:true; deepSemantic:boolean} };
export type AnalysisResult = { manifest:RepositoryManifest; nodes:LogisticsNode[]; edges:LogisticsEdge[]; flows:SystemFlow[]; principles:CodebasePrinciple[]; preferences:RepositoryPreference[]; findings:WhiteboxFinding[]; dependencies:DependencyHealth[]; snapshot:LivingSchemaSnapshot; editorDiagnostics:EditorDiagnostic[] };
