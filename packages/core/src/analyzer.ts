...
export async function analyzeRepository(input: { repositoryKey: string; files: RepositoryFile[]; goal: string; runId?: string; dashboardUrl?: string; previousVersion?: number }): Promise<AnalysisResult> {
  ...
  const externalFiles = files.filter((file) => /\b(fetch|axios\.|requests\.|http\.|https\.)/.test(file.content));
  ...
  for (const file of externalFiles) { const line = firstMatchingLine(file, /\b(fetch|axios\.|requests\.|http\.|https\.)/); edges.push({ id: stableId('edge', `${file.path}:http`), from: stableId('module', file.path), to: externalId, kind: 'calls', evidence: [numberedEvidence(file, line, 'Outbound HTTP call')], confidence: .9 }); }
  const orderedNodeIds = [...new Set([...nodes.filter((node) => node.kind === 'entry_point').map((node) => node.id), ...externalFiles.map((file) => stableId('module', file.path)), ...(externalFiles.length ? [externalId] : []), ...envs.map((env) => stableId('env', env.name))])];
  ...
  for (const file of externalFiles) {
    const hasTimeout = /AbortSignal\.timeout|timeout\s*:|Timeout\(|timeout=/.test(file.content); const line = firstMatchingLine(file, /\b(fetch|axios\.|requests\.|http\.|https\.)/); const evidence = [numberedEvidence(file, line, hasTimeout ? 'HTTP call includes bounded timeout handling' : 'HTTP call has no local timeout evidence')];
